...
public class Multiplier extends AbstractRobot implements IArithmeticalTask, IKillYourself {
	private volatile ILogStorage logs;
	private volatile ITaskStorage tasks;

	public Multiplier(ITaskStorage tasks, ILogStorage logs) {
		super(tasks, logs);
		this.tasks = tasks;
		this.logs = logs;
	}
	...
	public boolean killYourself() {
		...
		logs.addStringLog("Robot with id=" + Thread.currentThread().getId() + " and type="
				+ Thread.currentThread().getName() + " has been killed by himself.");
	...
	public void doArithmeticalTask(List<Integer> digits) {
		...
		stringBuffer.append(" = ").append(mult);
	...
	public void run() {
		while (true) {
			...
			/*
			 * List<Integer> digits =
			 * tasks.getArithmeticalTask(Thread.currentThread().getName(),
			 * Thread.currentThread().getId()); if (digits != null) {
			 * doArithmeticalTask(digits); }
			 */
		}
